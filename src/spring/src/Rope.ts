...
const gravityVector = new Vector2(0, 9.8);

class Mass extends PIXI.Container {
  massNum: number;
  pinned: boolean;
  forceVertor = new Vector2(0, 0);
  speedVector = new Vector2(0, 0);
  preMass: Mass | null;

  // 用于绘制质点的 PIXI.Graphics 对象
  ...
  update(dt) {
    ...
    if (this.pinned) this.forceVertor = new Vector2(0, 0);
...
class Spring {
  ...
  restLength: number;
  // TODO: 在这里添加 Spring 类的定义

  // 用于绘制弹簧的 PIXI.Graphics 对象
  private graphics: PIXI.Graphics;
  angleDeg: number;
  springGra: PIXI.Graphics;
  dampingNum: number = 3;
  ...
  updatePos() {
    const springDown = new Vector2(0, 0).sub(
      new Vector2(this.mass1.x - this.mass2.x, this.mass1.y - this.mass2.y)
    );
  ...
  update(dt) {
    ...
    let springDown = new Vector2(0, 0).sub(springUp);
    let curLen = springDown.length();
    if (curLen > 51) {
      this.updatePos();
      springUp = new Vector2(
        this.mass1.x - this.mass2.x,
        this.mass1.y - this.mass2.y
      );
      springDown = new Vector2(0, 0).sub(springUp);
      curLen = 50;
      this.mass2.forceVertor = new Vector2(0, 0)
...
class Rope extends PIXI.Container {
  ...
  constructor(
    ...
    this.restLen = new Vector2(
      this.masses[0].x - this.masses[1].x,
      this.masses[0].y - this.masses[1].y
    ).length();
  ...
  drawSpring(springGra: PIXI.Graphics) {
  ...
  simulateEuler(delta_t: number, gravity: PIXI.Point): void {
  ...
  simulateVerlet(delta_t: number, gravity: PIXI.Point): void {
    for (const spring of this.springs) {
